...
params:
  fichier_bacteries: "TP2_Regression_IC/data/bacterial_genomes.txt"
  fichier_sequoia: "TP2_Regression_IC/data/sequoia.txt"
  fichier_tomatos: "TP2_Regression_IC/data/tomatos_cherry.txt"
...
# Dossiers
path_data   <- here("TP2_Regression_IC", "data")
path_output <- here("TP2_Regression_IC", "output")
dir.create(path_output, showWarnings = FALSE, recursive = TRUE)

# Racine knit = racine du repo
knitr::opts_knit$set(root.dir = here::here())

list# Options globales (une seule fois)
knitr::opts_chunk$set(
  echo = TRUE, warning = FALSE, message = FALSE,
  fig.width = 6, fig.height = 4, dpi = 150,
  fig.path = file.path(path_output, "")  # toutes les figures dans output/
)
...
# Bactéries
f_bact <- here::here(params$fichier_bacteries)
stopifnot(file.exists(f_bact))
genomes <- read.table(
  f_bact,
  sep = params$separateur_bacterias, header = TRUE,
  comment.char = "", quote = ""
)

# Séquoias
f_seq <- here::here(params$fichier_sequoia)
stopifnot(file.exists(f_seq))
data_sequoia <- read.table(
  f_seq,
  sep = params$separateur_sequoia, header = TRUE,
  comment.char = "", quote = ""
)

# Tomates
f_tom <- here::here(params$fichier_tomatos)
stopifnot(file.exists(f_tom))
data_tomatos <- read.table(
  f_tom,
  sep = params$separateur_tomatos, header = TRUE,
  comment.char = "", quote = ""
)
```
...
names(genomes)
with(genomes , plot(Genome_size , ORF_number , pch=16 , ylab="Nombre d'ORF" , xlab = "Taille du génome (Mpb)" , main="Relation de la taille des genomes avec le nombre d'ORF. " ))
...
relation_Genome_size_nb_ORF = with(genomes , plot(Genome_size , ORF_number , pch=16 , ylab="Nombre d'ORF" , xlab = "Taille du génome (Mpb)" , main="Relation de la taille des genomes avec le nombre d'ORF. " ))
...
```{r Export_figures}
# figure régression
outfile_bacterias <- file.path(path_output, "relation_Genome_size_nb_ORF.jpg")
jpeg(outfile_bacterias)
relation_Genome_size_nb_ORF()
dev.off()

# histogramme séquoias
outfile_seq <- file.path(path_output, "histogramme_taille_arbre.jpg")
jpeg(outfile_seq)
histogramme_taille_arbre()
dev.off()

# loi binomiale
outfile <- file.path(path_output, "figure_loi_binomiale.jpg")
jpeg(outfile)
