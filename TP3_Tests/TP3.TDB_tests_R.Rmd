...
# Dossiers (figures / sorties)
path_output <- here("TP3_Tests", "output")
...
- L'hypothese nulle de ce test est que les frequences observées et les fréquences théoriques sont égales.  
...
mean_col <- mean(Col_0); mean_mut <- mean(Mut_X1)
sd_col   <- sd(Col_0);   sd_mut   <- sd(Mut_X1)

cat(sprintf("Moyennes: Col_0=%.2f ; Mut_X1=%.2f\nSD: Col_0=%.2f ; Mut_X1=%.2f\n",
            mean_col, mean_mut, sd_col, sd_mut))
```
...
par(mfrow = c(1,2))
hist(Col_0,  col = "gray90", main = "Histogramme Col_0", xlab = "Taille (cm)")
hist(Mut_X1, col = "gray90", main = "Histogramme Mut_X1", xlab = "Taille (cm)")

par(mfrow = c(1,1))
boxplot(Col_0, Mut_X1, names = c("Col_0","Mut_X1"),
        col = c("white","lightgreen"), ylab = "Taille (cm)", main = "Taille par groupe")
abline(h = mean_col, col = "black", lty = 3)
abline(h = mean_mut, col = "darkgreen", lty = 3)
```
...
```{r}
sh_col <- shapiro.test(Col_0)
sh_mut <- shapiro.test(Mut_X1)
sh_col$p.value; sh_mut$p.value

f_test <- var.test(Col_0, Mut_X1)  # test de Fisher
f_test
```
...
```{r, Test_Fischer}
var.test(Col_0, Mut_X1) # On teste l'homogénéité de la variance. 
...
```{r}
t_bi  <- t.test(Col_0, Mut_X1, var.equal = (f_test$p.value > 0.05))
t_gt  <- t.test(Col_0, Mut_X1, var.equal = (f_test$p.value > 0.05), alternative = "greater")
t_lt  <- t.test(Col_0, Mut_X1, var.equal = (f_test$p.value > 0.05), alternative = "less")

t_bi
t_gt$p.value; t_lt$p.value
```
...
Les hypothese alternatives on peut chosiir qu'elle soit unilatérale ou bilatérale. Mais ce choix doit être fait a priori et pas a posteriori. 
